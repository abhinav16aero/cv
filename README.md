...
We are using OpenCV 4.3.0 unavailable via `pip` and OpenCV for Java. That is why, we first need to build the OpenCV library. 
...
The proposed for the experiments ``Mobilenetv2ToOnnx.py`` script supports the ``--input_image`` key to customize the model conversion pipeline. It defines the full input image path, including its name - ``"coffee-mug.jpg"`` by default.
...
```bash
python3 Mobilenetv2ToOnnx.py --input_image "images/coffee-mug.jpg"
```
